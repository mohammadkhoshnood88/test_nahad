    ...
    <section class="content">
        <div class="content__inner">
            <header class="content__title">
                <h1 class="text-right">لیست لوکس فروشی </h1>
            </header>
                ...
                <div class="table-responsive text-center dir_rtl">
                    ...
                    @foreach($luxes as $lux)
                        <tr>
                            ...
                            <th scope="row">{{$lux->name}}</th>
                            <td>{{$lux->state->name}}-{{$lux->city->name}}</td>
                            <td>{{$lux->address}}</td>
                            <td>{{$lux->phone_number}}</td>
                            <td>
                            <img style="height: 100px;width: 100px" src="/post_images/related_images_watermark/{{$lux->main_img}}">
                            </td>
                            <td>
                                <button id="remove-lux" class="btn btn-outline-danger btnSwall remove_intro" style="font-family: 'YJ'">حذف</button> 
                                <a id="edit-lux" href="/admin/lux/{{$lux->id}}/edit" class="btn btn-outline-success btnSwall edit_intro" style="font-family: 'YJ'">ویرایش</a>
                            </td>
                        </tr>
                    @endforeach
